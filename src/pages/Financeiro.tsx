...
export function Financeiro() {
  ...
  const fluxoAcumulado = useMemo(() => {
    ...
    return (fluxoMensal as any[]).map((m: any) => {
      ...
      return {
        mes: m.mes
          ? format(parseISO(m.mes.substring(0, 10)), 'MMM/yy', { locale: ptBR })
          : '—',
        receitas: rec,
        despesas: desp,
        saldo_mes,
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="transacoes" className="space-y-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <Button variant="outline" size="icon" onClick={() => { setMesAtual(prev => subMonths(prev, 1)); setDataInicio(undefined); setDataFim(undefined); setPage(0) }}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <h3 className="text-lg font-bold capitalize">
              {format(mesAtual, "MMMM 'de' yyyy", { locale: ptBR })}
            </h3>
            <Button variant="outline" size="icon" onClick={() => { setMesAtual(prev => addMonths(prev, 1)); setDataInicio(undefined); setDataFim(undefined); setPage(0) }}>
