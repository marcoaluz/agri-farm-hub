...
export async function exportarNotaFiscalPDF(opts: {
  ...
  const { nomeArquivo, propriedadeNome, proprietarioNome, periodoLabel, modo, comNf, semNf } = opts
  const doc = new jsPDF()
  const pageWidth = doc.internal.pageSize.getWidth()
  const pageHeight = doc.internal.pageSize.getHeight()
  const margin = 14
  ...
  const tituloRelatorio =
    modo === 'com_nf' ? 'Relatório: Transações com Nota Fiscal'
    : modo === 'sem_nf' ? 'Relatório: Transações sem Nota Fiscal'
    : 'Relatório: Transações com e sem Nota Fiscal'
  doc.text(tituloRelatorio, textX, 20)
  doc.setFontSize(10)
  doc.text(`Propriedade: ${propriedadeNome}`, margin, 30)
  let yCabecalho = 36
  if (proprietarioNome) {
  ...
  const mostrarComNf = modo === 'ambos' || modo === 'com_nf'
  const mostrarSemNf = modo === 'ambos' || modo === 'sem_nf'
  ...
  if (mostrarComNf) {
    ...
    autoTable(doc, {
      ...
      body: comNf.length > 0
        ...
        : [['—', 'Nenhuma transação com nota fiscal neste período', '', '', '', '']],
  ...
  if (mostrarSemNf) {
    if (mostrarComNf) { doc.addPage(); desenharMarcaDagua(); y = 20 }
    ...
    autoTable(doc, {
      ...
      body: semNf.length > 0
        ...
        : [['—', 'Nenhuma transação sem nota fiscal neste período', '', '', '']],
